...
#from FI_unet import *
from esue_unet import *
#from data_generator import batch_generator, kitti_batch_generator
from esue_generator import batch_generator, normalize, zero_center
...
def main():
    ...
    X_val = np.load(os.path.join(VAL_FOLDER, "x/00000.npy")).astype("float32")
    y_val = np.load(os.path.join(VAL_FOLDER, "y/00000.npy")).astype("float32")
    X_val = zero_center(normalize(X_val))
    y_val = zero_center(normalize(y_val))
    ...
    # model = get_unet()
    model = get_unet_2(input_shape=(512, 512, 2))
